...
const SUPABASE_PUBLIC_API_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJueWhyd3dveWNycXl1c25tcHV5Iiwicm9sZSI6ImFub24iLCJpYXQiOjE2NTkwNzkwNjAsImV4cCI6MTk3NDY1NTA2MH0.kT5T8-HmkfHg75a-DDJ18baizzn_Qbuva0MHBs2NO98";
...
function getPlayers(): RegExpMatchArray[] {
  ...
  return Array.from(info.text().matchAll(/\S* \S*#([0-9]{4}|None)\s+\—\s+([a-zA-Z])*/gi));
}

async function loadTopshares(players: RegExpMatchArray[]): Promise<void> {
  let index = 0;

  const topshares: number[] = await Promise.all(players.map(async (player) => 
